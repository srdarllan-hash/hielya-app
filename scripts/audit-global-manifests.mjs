...
const required = ['components.json','primitives.json','assets.json','screens.json','certifications.json','workflows.json','artifacts.json','versions.json','C-005.json','C-001.json','C-001-visual-baseline.json','C-002.json','C-002-assets.json','C-002-visual-baseline.json'];
...
const result = {
  ...
  checks: {
    ...
    c002ApprovedFrozen: c002.status === 'APPROVED_FROZEN'
      && c002.approval?.approvedFrozen === true
      && c002.approval?.workIntegrated === true
      && screens.screens.find((item) => item.screenId === 'C-002')?.sourceStatus === 'APPROVED_FROZEN',
    ...
    componentVersion: components.version === '1.3.0' && versions.componentLibrary.gate2Candidate === '1.3.0',
